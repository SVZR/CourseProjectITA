...
public final class CoinDao {
    ...
    private Coin createCoinFromResultSet(ResultSet resultSet) throws SQLException {
        Coin coin = new Coin(
                ...
                LocalDate.parse(resultSet.getString(COINS_TABLE_NAME + ".releasedate")),
    ...
    public List<Coin> findAllCoins() {
        ...
        try (Connection connection = ConnectionManager.getConnection()) {
            try (PreparedStatement preparedStatement = connection.prepareStatement(
                    "SELECT c.id, c.name FROM coin c;")) {
                try (ResultSet resultSet = preparedStatement.executeQuery()) {
                    while (resultSet.next()) {
                        Coin coin = new Coin();
